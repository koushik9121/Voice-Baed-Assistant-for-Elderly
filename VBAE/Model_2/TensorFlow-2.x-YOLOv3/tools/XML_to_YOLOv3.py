...
def ParseXML(img_folder, file):
    for xml_file in glob.glob(img_folder+'/*/*.xml'):
        ...
        #img_path = img_folder+'/Scissors/'+image_name
        # Change is_subfolder at the start of this code to true and uncomment the above line and remove the Scissors/ from it


        img_path = str(root.find('path').text)        
